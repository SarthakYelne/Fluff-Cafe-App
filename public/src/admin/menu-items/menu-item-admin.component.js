(function () {
  ...
  angular.module("admin").component("menuItemAdmin", {
  ...
  function MenuItemAdminController(MenuService, ApiPath, Upload) {
    ...
    $ctrl.saveMenuItem = function () {
      return MenuService.saveMenuItem($ctrl.menuItem).then(
        function (menuItem) {
    ...
    $ctrl.validatePrices = function () {
      ...
      if ($ctrl.menuItem.price_small && $ctrl.menuItem.price_large) {
        if (
          !$ctrl.menuItem.small_portion_name ||
          !$ctrl.menuItem.large_portion_name
        ) {
          $ctrl.priceErrorMessage =
            "Portion names are required for both prices";
          return false;
        }
      }
      ...
      if ($ctrl.menuItem.small_portion_name && !$ctrl.menuItem.price_small) {
        $ctrl.priceErrorMessage =
          "If portion is filled in, an associated price must be entered";
        return false;
      }
